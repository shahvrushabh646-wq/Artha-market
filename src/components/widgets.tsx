...
import { CompanyInsightsGujarati } from "./company-insights-gujarati";
...
export function QuoteHeader({ quote }: { quote: Quote }) { return <><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><h1 className="font-display text-2xl tracking-tight text-fg sm:text-3xl">{quote.name}</h1><p className="mt-1 text-sm text-muted">{quote.symbol}{quote.exchange ? ` · ${quote.exchange}` : ""}</p></div><PriceBlock price={quote.price} change={quote.change} changePct={quote.changePct} size="lg" /></div><CompanyInsightsGujarati symbol={quote.symbol} companyName={quote.name}/></>; }
