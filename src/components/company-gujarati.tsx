...
async function getJson<T>(url: string): Promise<T> {
  const res = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "Artha-market/1.0" }, cache: "no-store" });
...
async function buildDescription(companyName: string, industry?: string | null): Promise<string> {
  ...
  try {
    ...
    translateUrl.search = new URLSearchParams({ client: "gtx", sl: "en", tl: "gu", dt: "t", q: extract.slice(0, 1400) }).toString();
    ...
    const result = parts.map((part) => Array.isArray(part) ? String(part[0] ?? "") : "").join("").trim() || fallback;
