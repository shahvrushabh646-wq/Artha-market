...
export function ShareholdingPledge({ symbol }: { symbol: string }) {
  const [shareTab, setShareTab] = useState<Tab>("Q1");
  const [pledgeTab, setPledgeTab] = useState<Tab>("Q1");
  ...
  const shareRows = shareTab === "5 YEARS" ? (data?.years ?? []) : (() => { const r = selectedQuarter(data?.quarters ?? [], shareTab); return r ? [r] : []; })();
  const pledgeRow = pledgeTab === "5 YEARS" ? (data?.years.filter(r => r.pledged != null) ?? []).at(-1) : selectedQuarter(data?.quarters ?? [], pledgeTab);
  ...
  return <section className="mt-8 space-y-7 border-t border-border pt-7">
    <div>
      ...
      {bars.length ? <div className="mt-3 rounded-xl bg-black px-1 py-4">
        <div className="mb-1 flex justify-between px-3 text-xs text-subtle"><span>{shareTab === "5 YEARS" ? "5-year annual comparison" : shareRows[0]?.period}</span><span>Percentage of total shares</span></div>
    ...
    <div className="border-t border-border pt-7">
      ...
      {pie.length ? <div className="mt-3 rounded-xl bg-black px-2 py-5">
        <div className="text-center text-xs text-subtle">{pledgeTab === "5 YEARS" ? "Latest annual pledge comparison" : (pledgeRow?.period ?? latestPledge?.period)}</div>
        <div className="mx-auto h-72 max-w-sm"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pie} dataKey="value" nameKey="name" cx="50%" cy="48%" innerRadius={70} outerRadius={112} label={({ value }) => pct(Number(value))} labelLine={false}>
          <Cell fill="#2563eb" /><Cell fill="#f97316" />
        </Pie><Tooltip formatter={(v: number) => pct(v)} contentStyle={{ background: "#17181d", border: "1px solid #303138", borderRadius: 10, color: "#fff" }} /></PieChart></ResponsiveContainer></div>
