...
async function createPgliteSql(): Promise<Sql> {
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    // Serverless deployments have no writable persistent application directory.
    // Use PGLite's explicit memory backend when DATABASE_URL is absent so Vercel
    // never tries to open the package's `_libs/pglite.data` file.
    const pg = new PGlite("memory://artha-preview", {
...
export function getSql(): Promise<Sql> {
  sqlPromise ??= createSql().catch((err) => { sqlPromise = null; throw err; });
  return sqlPromise;
}
export async function getPglite(): Promise<import("@electric-sql/pglite").PGlite> {
  if (dbSource !== "pglite") throw new Error("getPglite() is only available without DATABASE_URL");
  await getSql();
  const pg = await globalRef.__pgliteInstance__;
