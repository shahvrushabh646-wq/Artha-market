...
type YahooMeta = {
  currency?: string; symbol?: string; exchangeName?: string; fullExchangeName?: string;
  ...
  longName?: string; shortName?: string; chartPreviousClose?: number; previousClose?: number;
};
...
function emptyQuote(symbol: string): Quote {
  return { symbol, name: displaySymbol(symbol), price: null, previousClose: null, change: null, changePct: null, currency: "INR", exchange: null, high52w: null, low52w: null, volume: null, dayHigh: null, dayLow: null, ok: false };
}
function makeQuote(meta: YahooMeta, fallback: string): Quote {
  ...
  return {
    ...
    name: string(meta.longName) ?? string(meta.shortName) ?? fallback,
    price, previousClose: previous, change, changePct,
    currency: string(meta.currency) ?? "INR",
    exchange: string(meta.fullExchangeName) ?? string(meta.exchangeName),
    high52w: number(meta.fiftyTwoWeekHigh), low52w: number(meta.fiftyTwoWeekLow),
    volume: number(meta.regularMarketVolume), dayHigh: number(meta.regularMarketDayHigh), dayLow: number(meta.regularMarketDayLow),
    ok: price != null,
  };
}
...
function parseYahoo(raw: unknown): { bars: Bar[]; meta: YahooMeta; dividends: Dividend[] } {
  ...
  if (divMap) for (const item of Object.values(divMap)) {
    const row = record(item), amount = number(row?.amount), date = number(row?.date);
...
export const fetchAnalysis = createServerFn({ method: "POST" }).validator((data: unknown) => z.object({ symbol: z.string().min(1).max(24), period: z.enum(CHART_PERIODS).default("1Y") }).parse(data)).handler(async ({ data }) => {
  const symbol = normalizeSymbol(data.symbol), spec = PERIOD_MAP[data.period];
  const fiveYPromise = yahooChart(symbol, "5y", "1d", true);
  const periodPromise = data.period === "1D" || data.period === "1W" ? yahooChart(symbol, spec.range, spec.interval) : Promise.resolve(null);
...
export const fetchWatchPack = createServerFn({ method: "POST" }).validator((data: unknown) => z.object({ symbols: z.array(z.string()).max(20) }).parse(data)).handler(async ({ data }) => {
  ...
  const packs = await Promise.all(symbols.map(async symbol => {
    try {
      const [one, five] = await Promise.all([yahooChart(symbol, "1y", "1d"), yahooChart(symbol, "5y", "1wk")]);
      return { symbol, bars1y: parseYahoo(one).bars, bars5y: parseYahoo(five).bars };
...
export const searchSymbols = createServerFn({ method: "POST" }).validator((data: unknown) => z.object({ q: z.string().min(1).max(40) }).parse(data)).handler(async ({ data }): Promise<SearchHit[]> => {
  ...
  return [{ symbol: normalized, name: normalized, exchange: "NSE" }];
});
