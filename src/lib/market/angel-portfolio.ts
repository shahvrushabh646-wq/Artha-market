...
async function ensurePortfolioAccounts(sql:Awaited<ReturnType<typeof getSql>>){
...
export const getPortfolioAccounts=createServerFn({method:"GET"}).handler(async()=>{
  const sql=await getSql(); await ensurePortfolioAccounts(sql);
...
export const addPortfolioTransaction=createServerFn({method:"POST"}).handler(async(ctx)=>{
  ...
  const sql=await getSql(); const account=await sql`select id from portfolio_accounts where id=${accountId}`; if(!account.length)throw new Error("Account not found");
