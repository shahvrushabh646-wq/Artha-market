...
function indiaDate(ts: number): string {
...
function currentWeekStart(): string {
  return weekStart(new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Kolkata", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date()));
}

function previousWeekStart(thisWeek: string): string {
  ...
  for (const host of ["query1.finance.yahoo.com", "query2.finance.yahoo.com"]) {
    try {
      ...
      for (let i = 1; i < closes.length; i += 1) {
        ...
        if (previous !== null && current !== null && previous > threshold && current <= threshold && ts != null) triggerDay = indiaDate(ts);
      }
      ...
      // Try the second Yahoo endpoint.
    }
  }

  historyCache.set(symbol, { exp: Date.now() + 15 * 60_000, value });
  return value;
}
...
async function scanSuggestions(): Promise<Quote[]> {
  ...
  // 100-at-a-time keeps the scan bounded but removes the old 20-request bottleneck.
