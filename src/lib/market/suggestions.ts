...
async function bseUniverse(): Promise<string[]> {
  ...
  for (const group of BSE_GROUPS) {
    try {
      ...
      // Continue with the remaining BSE groups.
    }
  }
  return [...out];
}
...
async function history(symbol: string) {
  for (const host of ["query1.finance.yahoo.com", "query2.finance.yahoo.com"]) {
    try {
      ...
      const highValues = row?.indicators?.quote?.[0]?.high ?? [];
      const closeValues = row?.indicators?.quote?.[0]?.close ?? [];
      const timestamps = row?.timestamp ?? [];
      const highs = highValues.map(num).filter((x): x is number => x !== null);
      const closes = closeValues.map(num);
      const price = num(row?.meta?.regularMarketPrice) ?? [...closes].reverse().find((x) => x !== null) ?? null;
  ...
  try {
    ...
    for (const batch of chunk(symbols, 20)) {
      const rows = await Promise.allSettled(
        batch.map(async (symbol) => ({ symbol, data: await history(symbol) })),
      );
    ...
    // Keep one exchange entry per company and prefer NSE over BSE.
    ...
    cache.data = [...byCompany.values()]
      .sort((a, b) => {
        const priority = (q: Suggestion) => {
          if (!q.triggerDay) return 3;
          const w = weekStart(q.triggerDay);
          if (w === thisWeek) return 0;
          if (w === previousWeek) return 1;
          return 2;
        };
