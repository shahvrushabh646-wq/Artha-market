...
async function getSuggestions(): Promise<Quote[]> {
  const symbols = [...new Set([...MOVERS_UNIVERSE, ...EXTRA_SYMBOLS])];
  const rows: Quote[] = [];
  // Yahoo's Spark endpoint supports multiple symbols in one historical request,
  // so Suggestions no longer performs one 5Y request per stock.
  for (const batch of chunk(symbols, 50)) {
    ...
    try {
      ...
      for (const item of raw.spark?.result ?? []) {
        ...
        const highs = response.indicators?.quote?.[0]?.high ?? [];
        const price = Number(meta.regularMarketPrice ?? closes.filter(v => v != null).at(-1));
        if (!Number.isFinite(price)) continue;
        const validHighs = highs.filter((v): v is number => typeof v === "number" && Number.isFinite(v));
        if (!validHighs.length) continue;
        const high5y = Math.max(...validHighs);
        const isLowPrice = price < 20;
        const threshold = high5y * (isLowPrice ? 0.10 : 0.25);
        ...
        rows.push({
          ...
          high5y,
          low5y: null,
          price75: Math.round(high5y * 0.25 * 100) / 100,
          signal75: "BUY",
          volume: null,
          dayHigh: null,
          dayLow: null,
