import type { Bar, StockScore, TechnicalSummary, Valuation } from "./types";

export function round2(n: number): number {
  return Math.round(n * 100) / 100;
}

export function pctBelowHigh(current: number | null, high: number | null): number | null {
  if (current == null || high == null || high === 0) return null;
  return round2(((high - current) / high) * 100);
}

export function pctAboveLow(current: number | null, low: number | null): number | null {
...
export function lastValid(series: number[]): number | null {
...
export function rsiInterpretation(value: number | null): string {
...
export function priceVsSma(price: number | null, smaValue: number | null): "Above" | "Below" | "At" | "N/A" {
  if (price == null || smaValue == null || Number.isNaN(smaValue)) return "N/A";
  if (price > smaValue) return "Above";
  if (price < smaValue) return "Below";
  return "At";
}
...
export function supportResistance(bars: Bar[], lookback = 60): { support: number | null; resistance: number | null } {
...
export function parseMetric(value: string | number | null | undefined): number | null {
  ...
  const m = s.match(/^([+-]?\d*\.?\d+)\s*(cr|lakh|l|%)?$/i);
  if (!m) return null;
  let n = Number.parseFloat(m[1]);
  if (!Number.isFinite(n)) return null;
  const u = (m[2] || "").toLowerCase();
  if (u === "cr") n *= 1e7;
  else if (u === "lakh" || u === "l") n *= 1e5;
  return n;
}
