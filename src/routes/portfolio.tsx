...
const writeLocal = (id: string, value: Tx[]) => { try { localStorage.setItem(storageKey(id), JSON.stringify(value)); } catch {} };
...
function PortfolioPage() {
  ...
  const txQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const all = [...readLocal(accountId), ...(remote as Tx[])];
      const unique = new Map<string, Tx>();
  ...
  const todayPnl = holdings.reduce((sum, h) => {
    ...
    return sum + (Number.isFinite(change) ? h.qty * change : 0);
  }, 0);
  const filtered = holdings.filter(h => {
    const term = search.trim().toLowerCase();
    return !term || displaySymbol(h.symbol).toLowerCase().includes(term) || h.symbol.toLowerCase().includes(term);
  });
  ...
  async function saveTransaction() {
    ...
    if (!accountId || !symbol || !Number.isFinite(quantity) || quantity <= 0 || !Number.isFinite(price) || price < 0 || !Number.isFinite(charges) || charges < 0 || !/^\d{4}-\d{2}-\d{2}$/.test(tx.tradeDate)) {
      toast("Enter valid transaction details"); return;
    }
    if (tx.side === "SELL") {
      const holding = holdings.find(h => h.symbol === symbol && h.exchange === tx.exchange);
      if (!holding || quantity > holding.qty + 0.000001) { toast(`Available quantity: ${holding ? trim(holding.qty) : "0"}`); return; }
    }
    try {
      ...
      setShowTx(false);
      await qc.invalidateQueries({ queryKey: ["portfolio-transactions", accountId] });
  ...
  async function addAccount() {
    ...
    try {
      const account = await addPortfolioAccount({ data: { name: newAccount.trim() } });
      setNewAccount(""); setShowAccount(false); setAccountId(account.id);
  ...
  async function renameAccount() {
    ...
    try {
      await renamePortfolioAccount({ data: { id: editing, name: editName.trim() } });
      setEditing(null); await qc.invalidateQueries({ queryKey: ["portfolio-accounts"] }); toast("Portfolio renamed");
  ...
  return <div className="mx-auto max-w-6xl pb-10">
    <header className="flex items-center justify-between gap-3 py-1">
      <div>
        <div className="flex items-center gap-2">
          <h1 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">Holdings</h1>
          {active && <button onClick={() => { setEditing(active.id); setEditName(active.name); }} className="text-sm text-muted hover:text-fg">{active.name}</button>}
        </div>
        <p className="mt-1 text-xs text-muted sm:text-sm">Track your equity investments</p>
      </div>
      <div className="flex items-center gap-2">
        <button onClick={() => openTransaction("BUY")} className="flex h-10 items-center gap-1.5 rounded-full border border-border bg-bg px-4 text-sm font-medium hover:bg-surface-2"><Plus className="size-4"/>Add</button>
        <button onClick={() => setShowAccount(true)} className="flex size-10 items-center justify-center rounded-full border border-border bg-bg hover:bg-surface-2" aria-label="Portfolio options"><MoreVertical className="size-5"/></button>
      </div>
    </header>
    ...
    {active && <>
      <section className="mt-4 overflow-hidden rounded-2xl bg-slate-800 text-white shadow-sm">
        <div className="p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className={`mt-2 text-sm font-medium ${totalPnl >= 0 ? "text-emerald-300" : "text-red-300"}`}>
                {totalPnl >= 0 ? "↑ Overall Profit" : "↓ Overall Loss"} {fmtCurrency(Math.abs(totalPnl))} <span className="text-slate-300">({returnPct === null ? "—" : `${returnPct >= 0 ? "+" : ""}${returnPct.toFixed(2)}%`})</span>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs text-slate-300">Today&apos;s P&amp;L</div>
              <div className={`mt-1 text-lg font-semibold ${todayPnl >= 0 ? "text-emerald-300" : "text-red-300"}`}>{todayPnl >= 0 ? "+" : "−"}{fmtCurrency(Math.abs(todayPnl))}</div>
            </div>
          </div>
          <div className="mt-6 grid grid-cols-2 gap-4 border-t border-white/10 pt-4 sm:grid-cols-4">
            ...
            <DarkMetric label="Unrealised P&amp;L" value={<Signed value={unrealised} as="currency" />} />
            <DarkMetric label="Realised P&amp;L" value={<Signed value={realised} as="currency" />} />
          </div>
        </div>
        <div className="grid grid-cols-2 border-t border-white/10">
          ...
          <button onClick={() => openTransaction("BUY")} className="border-l border-white/10 py-3 text-sm font-medium text-cyan-300">+ BUY / SELL</button>
        </div>
      </section>

      <nav className="mt-5 flex gap-7 overflow-x-auto border-b border-border">
      ...
      {(tab === "overview" || tab === "equity") && <>
        <div className="mt-4 flex items-center gap-2 rounded-xl border border-border bg-bg px-3 py-2.5">
          <Search className="size-5 shrink-0 text-muted"/>
          <input value={search} onChange={e => setSearch(e.target.value)} className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted" placeholder="Search stocks by company"/>
          <SlidersHorizontal className="size-4 shrink-0 text-muted"/>
        </div>
        <div className="mt-3 flex items-center justify-between px-1 text-xs text-muted"><span>{filtered.length} holding{filtered.length === 1 ? "" : "s"}</span><span>{quotesQuery.isFetching ? "Updating…" : "Live prices"}</span></div>
        <Holdings rows={filtered} quotes={quotes} onBuy={h => openTransaction("BUY", h)} onSell={h => openTransaction("SELL", h)} />
      </>}
    ...
    {showAccount && <Modal title="Portfolio" close={() => setShowAccount(false)}>
      <input autoFocus value={newAccount} onChange={e => setNewAccount(e.target.value)} onKeyDown={e => { if (e.key === "Enter") void addAccount(); }} className="w-full rounded-lg border bg-bg px-3 py-2 text-sm" placeholder="New portfolio name"/>
      <div className="mt-4 flex justify-end gap-2"><Button variant="secondary" onClick={() => setShowAccount(false)}>Cancel</Button><Button onClick={addAccount}>Add Portfolio</Button></div>
    </Modal>}

    {showTx && <Modal title={tx.side === "BUY" ? "Buy Stock" : "Sell Stock"} close={() => setShowTx(false)}>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => setTx({ ...tx, side: "BUY" })} className={`rounded-lg border py-2.5 text-sm font-semibold ${tx.side === "BUY" ? "border-up bg-up/10 text-up" : "border-border text-muted"}`}>BUY</button>
          <button onClick={() => setTx({ ...tx, side: "SELL" })} className={`rounded-lg border py-2.5 text-sm font-semibold ${tx.side === "SELL" ? "border-down bg-down/10 text-down" : "border-border text-muted"}`}>SELL</button>
        </div>
        <input value={tx.symbol} onChange={e => setTx({ ...tx, symbol: e.target.value.toUpperCase() })} className="w-full rounded-lg border bg-bg px-3 py-2.5 text-sm" placeholder="Stock symbol e.g. TCS"/>
        <div className="grid grid-cols-2 gap-2">
          <select value={tx.exchange} onChange={e => setTx({ ...tx, exchange: e.target.value })} className="rounded-lg border bg-bg px-3 py-2.5 text-sm"><option>NSE</option><option>BSE</option></select>
          <input type="date" value={tx.tradeDate} onChange={e => setTx({ ...tx, tradeDate: e.target.value })} className="rounded-lg border bg-bg px-3 py-2.5 text-sm"/>
        </div>
        {tx.side === "SELL" && tx.symbol && <div className="rounded-lg bg-surface-2 px-3 py-2 text-xs text-muted">Available: {trim(holdings.find(h => h.symbol === tx.symbol && h.exchange === tx.exchange)?.qty || 0)} shares</div>}
        <div className="grid grid-cols-2 gap-2">
          <input type="number" min="0.0001" value={tx.quantity} onChange={e => setTx({ ...tx, quantity: e.target.value })} className="rounded-lg border bg-bg px-3 py-2.5 text-sm" placeholder="Quantity"/>
          <input type="number" min="0" step="0.01" value={tx.price} onChange={e => setTx({ ...tx, price: e.target.value })} className="rounded-lg border bg-bg px-3 py-2.5 text-sm" placeholder="Price"/>
        </div>
        <input type="number" min="0" step="0.01" value={tx.charges} onChange={e => setTx({ ...tx, charges: e.target.value })} className="w-full rounded-lg border bg-bg px-3 py-2.5 text-sm" placeholder="Charges (optional)"/>
        <Button className="w-full" onClick={saveTransaction}>{tx.side === "BUY" ? "Add BUY" : "Confirm SELL"}</Button>
      </div>
    </Modal>}
  </div>;
}
...
function TopTab({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return <button onClick={onClick} className={`whitespace-nowrap border-b-2 px-1 py-3 text-sm font-semibold transition-colors ${active ? "border-fg text-fg" : "border-transparent text-muted hover:text-fg"}`}>{label}</button>;
}

function Holdings({ rows, quotes, onBuy, onSell }: { rows: Holding[]; quotes: Map<string, Quote>; onBuy: (h: Holding) => void; onSell: (h: Holding) => void }) {
  ...
  return <div className="mt-3 space-y-3">{rows.map(h => {
    const ltp = quotes.get(h.symbol)?.price ?? h.avg;
    const value = h.qty * ltp;
    const pnl = value - h.invested;
    const pnlPct = h.invested ? pnl / h.invested * 100 : null;
    return <Panel key={`${h.exchange}:${h.symbol}`} className="overflow-hidden transition-shadow hover:shadow-sm">
      <Link to="/stock" search={{ symbol: h.symbol, period: "1Y" }} className="block p-4 pb-3">
        <div className="flex items-start justify-between gap-4">
          <div><div className="text-[16px] font-semibold tracking-tight">{displaySymbol(h.symbol)}</div><div className="mt-1 text-xs text-muted">{trim(h.qty)} shares · Avg. {fmtCurrency(h.avg)}</div></div>
          <div className="text-right"><Signed value={pnl} as="currency" className="text-[15px] font-semibold"/><div className="mt-0.5 text-xs"><Signed value={pnlPct} as="percent"/></div></div>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-4 sm:grid-cols-3">
          <Metric label="LTP" value={fmtCurrency(ltp)}/>
          <Metric label="Current value" value={fmtCurrency(value)}/>
          <Metric label="Invested" value={fmtCurrency(h.invested)}/>
        </div>
      </Link>
      <div className="grid grid-cols-2 border-t border-border">
        <button onClick={() => onBuy(h)} className="py-3 text-sm font-semibold text-up hover:bg-up/5">BUY</button>
        <button onClick={() => onSell(h)} className="border-l border-border py-3 text-sm font-semibold text-down hover:bg-down/5">SELL</button>
      </div>
    </Panel>;
  })}</div>;
}
...
function Transactions({ rows, onDelete, emptyTitle, emptyBody }: { rows: Tx[]; onDelete: (id: string) => void; emptyTitle: string; emptyBody: string }) {
  if (!rows.length) return <div className="mt-4"><Panel className="p-8"><Empty title={emptyTitle} body={emptyBody}/></Panel></div>;
  return <div className="mt-4 space-y-2">{rows.map(t => <Panel key={t.id} className="overflow-hidden">
    <div className="flex items-start justify-between gap-3 p-4">
      <div><div className="flex items-center gap-2"><span className={`rounded-md px-2 py-1 text-[11px] font-bold ${t.side === "BUY" ? "bg-up/10 text-up" : "bg-down/10 text-down"}`}>{t.side}</span><span className="text-[15px] font-semibold">{displaySymbol(t.symbol)}</span></div><div className="mt-1.5 text-xs text-muted">{t.exchange} · {formatDate(t.tradeDate)}</div></div>
      <button onClick={() => onDelete(t.id)} className="rounded-md p-1.5 text-muted hover:text-down" aria-label="Delete transaction"><Trash2 className="size-4"/></button>
    </div>
    <div className="grid grid-cols-3 gap-3 border-t border-border px-4 py-3"><Metric label="Quantity" value={trim(t.quantity)}/><Metric label="Price" value={fmtCurrency(t.price)}/><Metric label="Value" value={fmtCurrency(t.quantity * t.price)}/></div>
  </Panel>)}</div>;
}
...
function buildHoldings(txs: Tx[]): Holding[] {
  ...
  for (const t of [...txs].sort((a, b) => a.tradeDate.localeCompare(b.tradeDate) || a.id.localeCompare(b.id))) {
    const key = `${t.exchange}:${t.symbol}`;
    const h = map.get(key) || { symbol: t.symbol, exchange: t.exchange, qty: 0, avg: 0, invested: 0, realized: 0 };
    if (t.side === "BUY") {
      const newQty = h.qty + t.quantity;
      h.avg = (h.avg * h.qty + t.price * t.quantity) / newQty;
      h.qty = newQty;
      h.invested = h.avg * h.qty;
    } else {
      const sellQty = Math.min(h.qty, t.quantity);
      h.realized += t.price * sellQty - h.avg * sellQty - t.charges;
