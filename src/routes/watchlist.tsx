    ...
    <div>
      ...
      <form
        ...
        onSubmit={(e) => {
          e.preventDefault();
          if (!raw.trim()) return;
          const added = addWatch(raw);
          toast(added ? `Added ${normalizeSymbol(raw)}` : "Already watching");
          setRaw("");
        }}
      >
        <Input
      ...
      <div className="mt-5 space-y-2">
        {rows.length === 0 ? (
          <Empty title="Nothing on the list" body="Add RELIANCE, TCS or any NSE name to start tracking." />
        ) : (
          rows.map((row) => (
            <Panel key={row.symbol} className="p-3">
              <div className="flex items-start justify-between gap-3">
                <Link to="/stock" search={{ symbol: row.symbol, period: "1Y" }} className="min-w-0">
                  <div className="font-medium text-fg">{displaySymbol(row.symbol)}</div>
                  <div className="truncate text-xs text-muted">{row.q?.name ?? row.symbol}</div>
                </Link>
                <div className="text-right">
                  <div className="tabular text-fg">{fmtCurrency(row.q?.price ?? null)}</div>
                  <Signed value={row.q?.changePct ?? null} as="percent" className="text-xs" />
                </div>
              </div>
              <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-muted">
                <span>
                  52W H <span className="tabular text-fg">{fmtCurrency(row.hl1.high ?? row.q?.high52w ?? null)}</span>
                </span>
                <span>
                  52W L <span className="tabular text-fg">{fmtCurrency(row.hl1.low ?? row.q?.low52w ?? null)}</span>
                </span>
                <span>
                  RSI <span className="tabular text-fg">{fmtNumber(row.r)}</span>
                </span>
                <span>
                  SMA20 <span className="tabular text-fg">{fmtCurrency(row.s20)}</span>
                </span>
                <span>
                  SMA50 <span className="tabular text-fg">{fmtCurrency(row.s50)}</span>
                </span>
                <span>
                  SMA200 <span className="tabular text-fg">{fmtCurrency(row.s200)}</span>
                </span>
              </div>
              <div className="mt-3 flex items-center justify-between">
                {row.signal ? <SignalBadge signal={row.signal} /> : <span className="text-xs text-subtle">No signal</span>}
                <Button variant="ghost" size="sm" className="h-10" onClick={() => removeWatch(row.symbol)}>
                  Remove
                </Button>
              </div>
            </Panel>
