...
function Suggestions() {
  const q = useQuery({
    ...
    queryFn: () => fetchQuotes({ data: { symbols: [...MOVERS_UNIVERSE] } }),
    staleTime: 60_000,
  });
    ...
    <div>
      ...
      <Section title="Rule triggers" hint="₹20 and above: 75% level · Below ₹20: 90% level">
        {q.isLoading ? (
          ...
          <Panel><p className="text-sm text-muted">Suggestions are temporarily unavailable. Try again.</p></Panel>
        ) : suggestions.length === 0 ? (
          <Panel><div className="flex items-center gap-3"><Lightbulb className="size-5 text-muted" /><p className="text-sm text-muted">No stocks in the current scanner universe are triggering the rule.</p></div></Panel>
        ) : (
          <div className="space-y-2">
            {suggestions.map((quote) => {
              const isLow = (quote.price ?? 0) < 20;
              const level = isLow ? (quote.high5y ?? 0) * 0.10 : (quote.high5y ?? 0) * 0.25;
              return <Link key={quote.symbol} to="/stock" search={{ symbol: quote.symbol, period: "1Y" }} className="block">
                <Panel className="p-3 transition hover:bg-surface-2"><div className="flex items-center justify-between gap-3"><div><div className="font-medium text-fg">{displaySymbol(quote.symbol)}</div><div className="mt-1 text-xs text-muted">{isLow ? "90% Rule" : "75% Rule"} · Trigger ≤ {fmtCurrency(level)}</div></div><div className="text-right"><div className="tabular text-sm text-fg">{fmtCurrency(quote.price)}</div><div className="mt-1 text-xs text-muted">5Y high {fmtCurrency(quote.high5y)}</div></div></div><div className="mt-2"><SignalBadge signal="BUY" /></div></Panel>
              </Link>;
            })}
          </div>
        )}
