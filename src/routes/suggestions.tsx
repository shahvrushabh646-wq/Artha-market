...
const STORAGE_KEY = "artha:suggestions:v4";
...
function Suggestions(){
  ...
  const q=useQuery({queryKey:["suggestions-scanner-v4",period,sector],queryFn:()=>fetchSuggestions({data:{period,sector}}),initialData:period==="current"&&sector==="All"&&saved.length?saved:undefined,staleTime:0,gcTime:24*60*60_000,refetchOnMount:"always",refetchOnWindowFocus:true,refetchInterval:30*60_000});
  const suggestions=q.data??saved;
  const today=new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Kolkata",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date());
  const todayTriggers=useMemo(()=>period==="current"?suggestions.filter(x=>x.triggerDate===today):[],[suggestions,today,period]);
  const todaySymbols=useMemo(()=>new Set(todayTriggers.map(x=>x.symbol)),[todayTriggers]);
  const otherStocks=useMemo(()=>period==="current"?suggestions.filter(x=>!todaySymbols.has(x.symbol)):suggestions,[suggestions,todaySymbols,period]);
  ...
  return <div>
    ...
    <Section title={period==="current"?"Today's triggers":"Matching triggers"} hint={period==="current"?"Newly crossed 75% / 90% rule today":"Most recent matching trigger first"}>{q.isLoading&&suggestions.length===0?<div className="space-y-2"><SkeletonBlock className="h-20"/><SkeletonBlock className="h-20"/></div>:todayTriggers.length===0&&period==="current"?<Panel><div className="flex items-center gap-3"><Lightbulb className="size-5 text-muted"/><p className="text-sm text-muted">No new stocks triggered the rule today.</p></div></Panel>:period!=="current"&&suggestions.length===0?<Panel><p className="text-sm text-muted">No stocks triggered the rule in this period.</p></Panel>:<div className="space-y-2">{todayTriggers.map(x=><StockRow key={x.symbol} quote={x} todayTrigger/>)}{period!=="current"&&suggestions.map(x=><StockRow key={x.symbol} quote={x} todayTrigger={false}/>)}</div>}</Section>
    {period==="current"&&<Section title="Other stocks achieving the rule" hint={`Currently at or below the rule level: ${otherStocks.length}`}>{q.isError&&suggestions.length===0?<Panel><p className="text-sm text-muted">Suggestions are temporarily unavailable. Try Refresh.</p></Panel>:otherStocks.length===0?<Panel><p className="text-sm text-muted">No other stocks are currently achieving the rule.</p></Panel>:<div className="space-y-2">{otherStocks.map(x=><StockRow key={x.symbol} quote={x} todayTrigger={false}/>)}</div>}</Section>}
    <p className="mt-6 text-xs text-subtle">Default view keeps the recent trigger scanner. Selecting a period searches historical 75% / 90% rule triggers; sector selection filters the matching stocks.</p>
  </div>;
}
