...
const STORAGE_KEY = "artha:suggestions:v11";
...
function readSavedSuggestions():Quote[]{if(typeof window==="undefined")return[];try{const raw=window.localStorage.getItem(STORAGE_KEY)||window.localStorage.getItem("artha:suggestions:v10");if(!raw)return[];const parsed=JSON.parse(raw) as unknown;return Array.isArray(parsed)?parsed as Quote[]:[]}catch{return[];}}
...
function Suggestions(){
  ...
  const q=useQuery({queryKey:["suggestions-scanner-v11",period],queryFn:()=>fetchSuggestions({data:{period,sector:"All"}}),initialData:period==="current"&&saved.length?saved:undefined,staleTime:30_000,gcTime:24*60*60_000,refetchOnMount:"always",refetchOnWindowFocus:true,refetchInterval:30*60_000});
  ...
  return <div>
    ...
    <div className="mt-5"><label className="relative block max-w-md"><span className="mb-1.5 block text-xs font-medium text-muted">Trigger period</span><select value={period} onChange={e=>setPeriod(e.target.value as typeof period)} className="h-13 min-h-13 w-full appearance-none rounded-xl bg-surface-2 px-4 text-base font-medium text-fg shadow-[var(--shadow-border)] outline-none"><option value="current">All</option>{PERIODS.map(x=><option key={x.value} value={x.value}>{x.label}</option>)}</select></label></div>
