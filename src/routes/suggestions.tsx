...
const STORAGE_KEY = "artha:suggestions:v8";
...
function Suggestions(){
  ...
  const q=useQuery({queryKey:["suggestions-scanner-v8",period],queryFn:()=>fetchSuggestions({data:{period,sector:"All"}}),initialData:period==="current"&&sector==="All"&&saved.length?saved:undefined,staleTime:0,gcTime:24*60*60_000,refetchOnMount:"always",refetchOnWindowFocus:true,refetchInterval:30*60_000});
  ...
  return <div>
    ...
    <Section title={period==="current"?"Today's triggers":"Matching triggers"} hint={period==="current"?"Today's triggers first · alphabetical order":"Most recent trigger first"}>{q.isLoading&&suggestions.length===0?<div className="space-y-2"><SkeletonBlock className="h-20"/><SkeletonBlock className="h-20"/></div>:suggestions.length===0?<Panel><div className="flex items-center gap-3"><Lightbulb className="size-5 text-muted"/><p className="text-sm text-muted">No stocks match this sector / trigger period.</p></div></Panel>:<div className="space-y-2">{todayTriggers.map(x=><StockRow key={x.symbol} quote={x} todayTrigger/>)}{period!=="current"&&suggestions.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(x=><StockRow key={x.symbol} quote={x} todayTrigger={false}/>)}</div>}</Section>
