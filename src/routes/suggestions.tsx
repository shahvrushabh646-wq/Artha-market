...
const STORAGE_KEY = "artha:suggestions:v7";
...
function Suggestions(){
  ...
  const q=useQuery({queryKey:["suggestions-scanner-v7",period],queryFn:()=>fetchSuggestions({data:{period,sector:"All"}}),initialData:period==="current"&&sector==="All"&&saved.length?saved:undefined,staleTime:0,gcTime:24*60*60_000,refetchOnMount:"always",refetchOnWindowFocus:true,refetchInterval:30*60_000});
  const allSuggestions=q.data??saved;
  const suggestions=useMemo(()=>sector==="All"?allSuggestions:allSuggestions.filter(x=>sectorFor(x)===sector),[allSuggestions,sector]);
  ...
  return <div>
    ...
    <Section title={period==="current"?"Today's triggers":"Matching triggers"} hint={period==="current"?"Today's triggers first · alphabetical order":"Most recent trigger first"}>{q.isLoading&&suggestions.length===0?<div className="space-y-2"><SkeletonBlock className="h-20"/><SkeletonBlock className="h-20"/></div>:todayTriggers.length===0&&period==="current"&&sector==="All"?<Panel><div className="flex items-center gap-3"><Lightbulb className="size-5 text-muted"/><p className="text-sm text-muted">No new stocks triggered the rule today.</p></div></Panel>:suggestions.length===0?<Panel><p className="text-sm text-muted">No stocks match this sector / trigger period.</p></Panel>:<div className="space-y-2">{todayTriggers.map(x=><StockRow key={x.symbol} quote={x} todayTrigger/>)}{period!=="current"&&suggestions.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(x=><StockRow key={x.symbol} quote={x} todayTrigger={false}/>)}</div>}</Section>
    ...
    <p className="mt-6 text-xs text-subtle">Sector / Industry is applied to the complete returned trigger pool, including today's triggers and other stocks achieving the rule. Trigger period is applied before the sector filter.</p>
  </div>;
}
