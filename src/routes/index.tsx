...
const fetchPreciousMetals = createServerFn({ method: "GET" }).handler(async () => {
  ...
  // Five-year reference highs use Yahoo Finance's continuous COMEX futures history.
  // This avoids consuming dozens of Metals.Dev historical requests (its timeseries endpoint is limited to 30 days per request).
  async function yahoo5y(symbol: string) {
    try {
      ...
      const data = await yr.json() as YahooChart; const highs = data.chart?.result?.[0]?.indicators?.quote?.[0]?.high ?? [];
      const valid = highs.map(num).filter((x): x is number => x !== null); return valid.length ? Math.max(...valid) : null;
    } catch { return null; }
  }
  const [gold5yUsd, silver5yUsd] = await Promise.all([yahoo5y("GC=F"), yahoo5y("SI=F")]);
  // Convert the five-year USD/oz reference high into INR using today's Metals.Dev INR conversion.
  const goldUsd = num(raw.metals?.gold); const silverUsd = num(raw.metals?.silver);
  const goldHigh10g = gold5yUsd != null && goldUsd != null ? goldPerGram * 10 * (gold5yUsd / goldUsd) : null;
  const silverHighKg = silver5yUsd != null && silverUsd != null ? silverPerGram * 1000 * (silver5yUsd / silverUsd) : null;
  return { gold10g: goldPerGram * 10, silverKg: silverPerGram * 1000, gold5yHigh10g: goldHigh10g, silver5yHighKg: silverHighKg, source: "Metals.Dev · MCX + Yahoo Finance 5Y reference", asOf: raw.timestamp ?? new Date().toISOString() };
});
function Home() {
...
function PreciousMetals() {
  const metals = useQuery({ queryKey: ["precious-metals-mcx-metalsdev-5y-v1"], queryFn: () => fetchPreciousMetals(), staleTime: 60 * 60_000, refetchInterval: 60 * 60_000, refetchOnWindowFocus: false, retry: 1, gcTime: 2 * 60 * 60_000 });
