...
/** Live MCX-linked metal prices. Keep the API key server-side in Vercel. */
const fetchPreciousMetals = createServerFn({ method: "GET" }).handler(async () => {
  ...
  try {
    ...
    const goldPerGram = num(raw.metals?.mcx_gold) ?? num(raw.metals?.mcx_gold_pm) ?? num(raw.metals?.mcx_gold_am);
    const silverPerGram = num(raw.metals?.mcx_silver) ?? num(raw.metals?.mcx_silver_pm) ?? num(raw.metals?.mcx_silver_am);
    if (goldPerGram == null || silverPerGram == null) throw new Error("MCX Gold/Silver not returned");
    return {
      gold10g: goldPerGram * 10,
      silverKg: silverPerGram * 1000,
      source: "Metals.Dev · MCX",
      asOf: raw.timestamp ?? new Date().toISOString(),
    };
  } catch (e) {
    throw e instanceof Error ? e : new Error("Unable to load MCX metal prices");
  }
});
...
function PreciousMetals() {
  const metals = useQuery({ queryKey: ["precious-metals-mcx-metalsdev-v1"], queryFn: () => fetchPreciousMetals(), staleTime: 15_000, refetchInterval: 30_000, retry: 2 });
