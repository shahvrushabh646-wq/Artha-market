...
async function getIndianMetalPrices(): Promise<{ goldUsdOz: number; silverUsdOz: number; goldChange24hPct: number | null; goldChange24hAmountUsdOz: number | null; silverChange24hPct: number | null; silverChange24hAmountUsdOz: number | null; asOf: string | null } | null> {
  try {
    ...
    const goldUsdOz = Number(goldMeta?.regularMarketPrice);
    const silverUsdOz = Number(silverMeta?.regularMarketPrice);
    const goldPrev = Number(goldMeta?.previousClose);
    const silverPrev = Number(silverMeta?.previousClose);
    if (![goldUsdOz, silverUsdOz].every(Number.isFinite) || goldUsdOz <= 0 || silverUsdOz <= 0) return null;
    return {
      goldUsdOz,
      silverUsdOz,
      goldChange24hAmountUsdOz: Number.isFinite(goldPrev) ? goldUsdOz - goldPrev : null,
      goldChange24hPct: Number.isFinite(goldPrev) && goldPrev > 0 ? ((goldUsdOz - goldPrev) / goldPrev) * 100 : null,
      silverChange24hAmountUsdOz: Number.isFinite(silverPrev) ? silverUsdOz - silverPrev : null,
      silverChange24hPct: Number.isFinite(silverPrev) && silverPrev > 0 ? ((silverUsdOz - silverPrev) / silverPrev) * 100 : null,
...
const fetchPreciousMetals = createServerFn({ method: "GET" }).handler(async (): Promise<MetalPrices> => {
  ...
  return { gold10g: goldUsdOz, silverKg: silverUsdOz, goldChange24hPct: quote.goldChange24hPct, goldChange24hAmount10g: quote.goldChange24hAmountUsdOz, silverChange24hPct: quote.silverChange24hPct, silverChange24hAmountKg: quote.silverChange24hAmountUsdOz, gold5yHigh10g, goldDiscount10Price10g, goldDiscount20Price10g, goldDiscount30Price10g, goldDiscount40Price10g, silver5yHighKg, silver25PriceKg, silver35PriceKg, silver45PriceKg, silver50PriceKg, silver55PriceKg, goldSignal: "WAIT", asOf: quote.asOf, source: "Yahoo Finance current Gold & Silver futures" };
});
