...
type YahooChart = { chart?: { result?: Array<{ timestamp?: number[]; indicators?: { quote?: Array<{ high?: Array<number | null> }> } }> } };
...
const fetchPreciousMetals = createServerFn({ method: "GET" }).handler(async () => {
  ...
  // Calculate the five-year high from DAILY historical benchmark prices in INR.
  // GC=F/SI=F are COMEX futures in USD per troy ounce; USDINR=X supplies the
  // corresponding historical INR conversion for each day. This avoids using
  // today's FX rate to back-convert an old USD high, which would give a false 5Y high.
  async function yahooSeries(symbol: string) {
    try {
      ...
      const r = await fetch(y, { cache: "no-store", headers: { "User-Agent": "Mozilla/5.0", Accept: "application/json" } }); if (!r.ok) return null;
      return (await r.json()) as YahooChart;
    } catch { return null; }
  }
  const [goldChart, silverChart, fxChart] = await Promise.all([yahooSeries("GC=F"), yahooSeries("SI=F"), yahooSeries("INR=X")]);
  const fxRow = fxChart?.chart?.result?.[0];
  const fxTs = fxRow?.timestamp ?? []; const fxClose = fxRow?.indicators?.quote?.[0]?.high ?? [];
  const fx = new Map<string, number>();
  for (let i = 0; i < fxTs.length; i++) { const v = num(fxClose[i]); if (v != null) fx.set(new Date(fxTs[i] * 1000).toISOString().slice(0, 10), v); }
  function convertedHigh(chart: YahooChart | null, grams: number) {
    const row = chart?.chart?.result?.[0]; if (!row) return null; const ts = row.timestamp ?? [], highs = row.indicators?.quote?.[0]?.high ?? []; let max: number | null = null;
    for (let i = 0; i < Math.min(ts.length, highs.length); i++) { const usd = num(highs[i]); const rate = fx.get(new Date(ts[i] * 1000).toISOString().slice(0, 10)); if (usd != null && rate != null) { const inr = usd * rate * grams / TROY_OZ_GRAMS; if (max == null || inr > max) max = inr; } }
    return max;
  }
  const goldHigh10g = convertedHigh(goldChart, 10);
  const silverHighKg = convertedHigh(silverChart, 1000);
  return { gold10g: goldPerGram * 10, silverKg: silverPerGram * 1000, gold5yHigh10g: goldHigh10g, silver5yHighKg: silverHighKg, source: "Metals.Dev current INR + Yahoo Finance 5Y daily benchmark", asOf: raw.timestamp ?? new Date().toISOString() };
});
...
function PreciousMetals() { const metals = useQuery({ queryKey: ["precious-metals-mcx-metalsdev-5y-v3"], queryFn: () => fetchPreciousMetals(), staleTime: 60 * 60_000, refetchInterval: 60 * 60_000, refetchOnWindowFocus: false, retry: 1, gcTime: 2 * 60 * 60_000 }); const gold = { name: "Gold", price: metals.data?.gold10g ?? null, high5y: metals.data?.gold5yHigh10g ?? null, unit: "₹ / 10g", discounts: [25,30,40,50] }; const silver = { name: "Silver", price: metals.data?.silverKg ?? null, high5y: metals.data?.silver5yHighKg ?? null, unit: "₹ / kg", discounts: [40,50,60,70] }; const formatINR = (value: number) => `₹${Math.round(value).toLocaleString("en-IN")}`; return <Section title="Gold & Silver" hint="Current price + true 5-year daily high reference"><div className="grid gap-3 sm:grid-cols-2">{[gold,silver].map(m => <Panel key={m.name} className="p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-medium text-fg">{m.name}</div><div className="mt-1 text-xs text-muted">MCX-linked · {m.unit}</div></div><div className="text-xs text-muted">INR</div></div><div className="mt-2 text-2xl font-semibold tabular text-fg">{m.price != null ? formatINR(m.price) : metals.isLoading ? "Loading…" : "Price unavailable"}</div><div className="mt-2 text-xs text-muted">5-year highest price: <span className="font-medium text-fg">{m.high5y != null ? formatINR(m.high5y) : "—"}</span></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{m.discounts.map(d => <div key={d} className="rounded-lg bg-surface-2 p-2"><div className="text-xs text-muted">{d}% discount</div><div className="mt-1 tabular text-sm text-fg">{m.high5y != null ? formatINR(m.high5y * (1-d/100)) : "—"}</div></div>)}</div></Panel>)}</div><p className="mt-2 text-[11px] text-subtle">5-year high is calculated from daily Gold/Silver futures highs with the historical USD/INR rate for the same day, then converted to ₹/10g and ₹/kg. Discount prices are calculated directly from that 5-year highest price. Current price refreshes hourly.</p></Section>; }
