...
const flag = (country: string) => ({ India: "🇮🇳", "United States": "🇺🇸", "United Kingdom": "🇬🇧", "Other countries": "🌍" }[country] ?? "🌍");
const countryNameGujarati = (country: string) => ({ India: "ભારત", "United States": "અમેરિકા", "United Kingdom": "યુનાઇટેડ કિંગડમ", "Other countries": "અન્ય દેશો" }[country] ?? country);
const gujaratiCountryBusiness = (business: string) => ({ Jewellery: "જ્વેલરી વ્યવસાય", "Healthcare services": "હેલ્થકેર સેવાઓ", "Luxury fashion": "લક્ઝરી ફેશન" }[business] ?? business);
  ...
  return <div>
    ...
    <div className="mt-5 space-y-3">
      {loading && !ipos.length ? <><SkeletonBlock className="h-32"/><SkeletonBlock className="h-32"/><SkeletonBlock className="h-32"/></> : ipos.map(ipo => <Link key={ipo.id} to="/ipo" search={{ id: ipo.id }} className="block"><Panel className="p-4 transition-colors hover:bg-surface-2">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2"><Mini label="Minimum application" value={money(ipo.minSubscription)} /><Mini label="Overall subscription" value={ipo.subscription == null ? "NSE data unavailable" : `${ipo.subscription}x`} /><Mini label="GMP" value={ipo.gmpPct == null ? "Not verified" : `+${ipo.gmpPct}%`} green={ipo.gmpPct != null} /></div>
      </Panel></Link>)}
    </div>
    <p className="mt-4 text-[11px] text-subtle">Overall subscription માટે Artha માત્ર official NSE figure બતાવે છે. Third-party websitesના અલગ timestampsના આંકડા mix અથવા average કરવામાં આવતા નથી.</p>
  </div>;
}
...
function IpoDetail({ id, ipos, loading, onRefresh }: { id: string; ipos: Awaited<ReturnType<typeof fetchOpenIpos>>; loading: boolean; onRefresh: () => void }) {
  ...
  return <div>
    ...
    <div className="mt-4 grid grid-cols-3 gap-2"><Panel className="p-4"><div className="text-xs text-muted">Closing Date</div><div className="mt-1 tabular text-lg text-accent">{date(ipo.closeDate)}</div></Panel><Panel className="p-4"><div className="text-xs text-muted">Overall Subscription</div><div className="mt-1 tabular text-lg font-semibold text-fg">{ipo.subscription == null ? "NSE data unavailable" : `${ipo.subscription}x`}</div><div className="mt-0.5 text-[10px] text-subtle">{ipo.subscriptionSource ?? "Official figure unavailable"}</div></Panel><Panel className="p-4"><div className="text-xs text-muted">GMP Today</div><div className={cn("mt-1 tabular text-lg font-semibold", ipo.gmpPct != null ? "text-up" : "text-muted")}>{ipo.gmpPct == null ? "Not verified" : `+${ipo.gmpPct}%`}</div><div className="mt-0.5 text-[10px] text-subtle">Unofficial</div></Panel></div>
    ...
    <Section title="IPO વિશે માહિતી">
      ...
      <div className="border-b border-border py-4"><div className="flex gap-3"><span className="flex size-7 shrink-0 items-center justify-center rounded-md bg-accent/15 text-xs font-semibold text-accent">4</span><div className="min-w-0 flex-1"><div className="text-sm font-medium text-fg">કયા દેશોમાં બિઝનેસ કરે છે અને વેચાણ કેટલું છે</div>{ipo.countries.length ? <div className="mt-3 overflow-hidden rounded-lg border border-border"><div className="grid grid-cols-[1.05fr_1.25fr_.7fr] bg-surface-2 px-3 py-2 text-[11px] font-semibold text-muted"><div>દેશ</div><div>બિઝનેસ</div><div className="text-right">વેચાણ %</div></div>{ipo.countries.map(c => <div key={c.country} className="grid grid-cols-[1.05fr_1.25fr_.7fr] items-center border-t border-border px-3 py-3 text-xs"><div className="flex items-center gap-2 font-medium text-fg"><span className="text-base">{flag(c.country)}</span><span>{countryNameGujarati(c.country)}</span></div><div className="text-muted">{gujaratiCountryBusiness(c.business)}</div><div className="text-right tabular font-semibold text-fg">{c.salesPct == null ? "—" : `${c.salesPct}%`}</div></div>)}</div> : <div className="mt-1 text-sm leading-6 text-muted">દેશવાર verified વેચાણની માહિતી ઉપલબ્ધ નથી.</div>}</div></div></div>
      <div className="border-b border-border py-4"><div className="flex gap-3"><span className="flex size-7 shrink-0 items-center justify-center rounded-md bg-accent/15 text-xs font-semibold text-accent">5</span><div className="min-w-0 flex-1"><div className="text-sm font-medium text-fg">છેલ્લા 3 વર્ષનો નફો</div>{ipo.profits.length ? <div className="mt-3 overflow-hidden rounded-lg border border-border"><div className="grid grid-cols-2 bg-surface-2 px-3 py-2 text-[11px] font-semibold text-muted"><div>નાણાકીય વર્ષ</div><div className="text-right">નફો</div></div>{ipo.profits.map(p => <div key={p.year} className="grid grid-cols-2 border-t border-border px-3 py-3 text-xs"><div className="font-medium text-fg">{p.year}</div><div className={cn("text-right tabular font-semibold", p.value != null && p.value < 0 ? "text-down" : "text-up")}>{p.value == null ? "માહિતી ઉપલબ્ધ નથી" : money(p.value)}</div></div>)}</div> : <div className="mt-1 text-sm leading-6 text-muted">છેલ્લા 3 વર્ષના verified profit figures ઉપલબ્ધ નથી.</div>}</div></div></div>
      <div className="border-b border-border py-4 last:border-b-0"><div className="flex gap-3"><span className="flex size-7 shrink-0 items-center justify-center rounded-md bg-accent/15 text-xs font-semibold text-accent">6</span><div className="min-w-0 flex-1"><div className="text-sm font-medium text-fg">પ્રાઇસ બેન્ડ અને ફાળવાતા શેર</div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-lg bg-surface-2 p-3"><div className="text-[10px] uppercase tracking-wide text-subtle">પ્રાઇસ બેન્ડ</div><div className="mt-1 text-sm tabular font-semibold text-fg">{ipo.priceBand ?? "માહિતી ઉપલબ્ધ નથી"}</div></div><div className="rounded-lg bg-surface-2 p-3"><div className="text-[10px] uppercase tracking-wide text-subtle">એક લોટમાં શેર</div><div className="mt-1 text-sm tabular font-semibold text-fg">{ipo.lotSize == null ? "માહિતી ઉપલબ્ધ નથી" : `${ipo.lotSize.toLocaleString("en-IN")} શેર`}</div></div></div></div></div></div>
    </Section>

    <Panel className="mt-4 p-4"><button type="button" onClick={() => setShowSources(v => !v)} className="text-sm font-medium text-fg">Subscription source policy {showSources ? "▲" : "▼"}</button>{showSources ? <div className="mt-3 space-y-2 text-xs text-muted"><p>1. Official NSE subscription multiple is the only number shown as Overall Subscription.</p><p>2. NSE and BSE publish live bid data separately; values from different timestamps are not averaged.</p><p>3. Groww, IPOWatch, IPO Ji and Mint may be used only for manual cross-checking, never to overwrite the official exchange figure.</p><p>4. If official NSE data is unavailable, Artha shows “NSE data unavailable” instead of guessing.</p></div> : null}</Panel>
    <p className="mt-4 text-[11px] text-subtle">Subscription is live bid-demand data and can change during the IPO window. Source: {ipo.subscriptionSource ?? "Official NSE data unavailable"}. Last check: {new Date(ipo.verifiedAt).toLocaleString("en-IN")}.</p>
  </div>;
}
